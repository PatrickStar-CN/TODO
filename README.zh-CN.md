...
<img src="./public/icon.svg" width="96" height="96" alt="TODO Tools 图标" />
...
**使用原生 JavaScript、CSS 和 HTML 构建的轻量待办事项工具，不依赖前端框架。通过 Vite 提供 Web 开发环境，并使用 Neutralinojs 打包 Windows 桌面应用。**

**[English](./README.md)** · [快速开始](#快速开始) · [功能概览](#功能概览) · [效果预览](#效果预览)
...
> 以下为项目真实运行截图（`npm run dev` + 演示数据）。桌面端在此界面基础上还提供迷你模式、系统托盘和自动更新。

| 任务工作区 | 月历视图 |
| --- | --- |
| ![任务工作区：侧栏视图、快捷添加、优先级、标签、已完成坞](./docs/assets/screenshot-tasks.png) | ![月历视图：任务标记与日期详情](./docs/assets/screenshot-calendar.png) |
| 侧栏视图（TODO / 重要 / 全部 / 归档 / 日历）、带日期–优先级–标签预设的快捷添加、优先级状态色、标签计数，以及底部固定的“已完成”悬浮坞。 | 月历网格带三档任务量标记、年份月份直选、图表区可折叠，日期详情与图表统计始终保持一致。 |

| 年度热力图 | AI 总结 | 外观设置 |
| --- | --- | --- |
| ![GitHub 风格年度任务量热力图](./docs/assets/screenshot-heatmap.png) | ![AI 总结面板：日报/周报/月报](./docs/assets/screenshot-ai.png) | ![外观设置：主题、圆角、玻璃、字体、动效、时间线](./docs/assets/screenshot-settings.png) |
| 任务量与完成量两种年度热力图，GitHub 风格年份网格 + 右下角强度图例。 | 日报 / 周报 / 月报（月报按自然月统计）流式生成，使用用户自配的接口地址。 | 跟随系统 / 亮色 / 暗色主题，圆角、玻璃透明度、字体大小、模糊强度、全局动画速度均可实时调整。 |
...
| 领域 | 要点 |
| --- | --- |
| 任务管理 | 快速添加、编辑、删除、完成/取消完成；优先级、标签、备注、开始时间、截止时间、提醒时间；每天/每周/每月重复提醒；归档/取消归档；批量清理已完成；按标题、备注、标签实时搜索 |
| 快捷操作 | 添加任务时可预设日期、优先级和标签（可在标签菜单中直接创建并选中新标签）；任务右键菜单支持重要、TODO、标签、提醒、归档；右键子菜单具有安全的指针移动区域，优先级和标签使用与任务列表一致的状态色 |
| 布局 | 已完成列表固定在任务区域底部，可悬浮展开/折叠、可拖拽调高；侧栏可折叠并保存折叠状态，折叠后为 `64px` 紧凑布局；迷你模式（`240 × 288`）带待办统计和快速添加入口；桌面版支持置顶迷你模式、系统托盘、关闭后隐藏到托盘 |
| 日历 | 月历 + 全年任务量热力图 + 全年完成量热力图；前后翻月、返回今天、直接选择年份月份；日期详情支持按天/按月切换（粘性分组头含日期、计数、今天标记与整组完成态，跨天任务带“持续至”徽章）；超过 25 项时底部进度条配合滚动自动加载或点击加载更多 |
| 外观与体验 | 克制的磨砂玻璃办公风界面，页面背景不使用渐变；图标统一由 `src/icons.js` 的 SVG 图标库渲染，不使用 Emoji；自定义滚动条绑定到对应滚动容器；下拉控件支持方向键、回车、空格和 `Esc`，菜单、日期和页签保留键盘操作与 ARIA 状态 |
| 设置 | 外观、AI 配置、提醒、标签管理、系统；AI 流式日报/周报/月报；桌面端自动更新（从 GitHub Releases 检查新版本，下载 zip 并校验 SHA-256 后替换应用文件，失败自动回滚） |
| 数据安全 | Web Crypto 可用时采用 AES-GCM 加密，旧版明文 JSON 和旧加密格式仍可兼容读取；无法解析或解密时暂停后续文件覆盖，并尽量备份带时间戳的 `.bak` 文件；`localStorage` 快照不保存 `aiConfig.apiKey`；内部 `_index` 只用于运行时计数，不写入持久化文件 |
...
> `npm run release -- <版本号>` 的版本号不带 `v` 前缀，且必须与 `package.json` 一致。

## 使用方式

1. **快速记录**——在快捷添加栏输入任务，不离键盘即可预设日期、优先级和标签。
2. **按视图分拣**——TODO 看今天、重要看星标、全部看所有、归档看已办结、日历按时间回顾。
3. **右键提速**——无需打开编辑面板，即可标记重要/TODO、改标签、设提醒或归档。
4. **日历回顾**——在月历、任务量、完成量之间切换，点击任意日期查看详情（不会自动切换图表模式）。
5. **AI 总结**——从日历右上角打开 *AI 总结*，选择日报 / 周报 / 月报，生成后一键复制。
6. **打磨外观**——设置 → 外观中实时调整主题、圆角、玻璃透明度、字体大小、模糊强度和动画速度；可选开启任务时间线，在列表右侧显示创建与完成时间并按其排序。
...
├── index.html
├── package.json
├── app.config.json             # 应用名称、窗口和迷你模式配置（配置源）
├── neutralino.config.json      # Neutralino 运行配置（由同步脚本生成）
├── vite.config.js              # Vite 构建和资源复制配置
├── server-plugin.js            # 开发环境数据读写接口（仅开发环境）
├── scripts/
...
<details>
<summary><strong>数据保护规则</strong></summary>
...
<details>
<summary><strong>数据模型</strong></summary>
...
- 按月详情与所选模式共用同一套规则：月历/任务量按任务覆盖范围聚合去重，完成量按 `doneAt` 落月筛选。
...
<details>
<summary><strong>材质与主题</strong></summary>
...
<details>
<summary><strong>外观设置联动</strong></summary>
...
<details>
<summary><strong>图标、按钮、布局、动效、焦点与可访问性</strong></summary>
...
- 动效应简短克制，并遵循 `prefers-reduced-motion`；桌面最小窗口按 `800 × 500` 检查。
- 不使用紫色亮边框或外发光作为常规焦点反馈；键盘状态应通过主题适配的边框、背景或文字颜色表达，避免破坏选中状态。
...
新增或修复业务规则时，请同步扩展 `scripts/check-state.js` 中的回归断言。修改范围保持最小并沿用现有实现：原生 ES Module 风格（2 空格缩进、单引号、分号），通用逻辑放入合适的现有模块或 `src/utils/`，避免继续扩大 `src/app.js`。

## 许可证

本仓库暂未附带 `LICENSE` 文件（`package.json` 中为 `"private": true`），默认保留所有权利。如需复用或分发代码，请先联系作者。

---

<div align="center">

原生 JavaScript · Vite · Neutralinojs 构建
