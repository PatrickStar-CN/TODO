...
- 月历格子任务标记按当天任务数分三档宽度，一眼分辨忙闲
- 任务量和完成量模式使用居中的 GitHub 风格年度热力图，前后导航用于切换年份
- 点击任意日期会更新下方任务列表，不会自动切换当前图表模式
- 日期详情支持按天/按月切换；按月查看整月任务（粘性分组头含日期、计数、今天标记与整组完成态，点击跳回单日，跨天任务带“持续至”徽章），标题显示星期与已完成统计，超过 25 项时底部进度条配合滚动自动加载或点击加载更多
