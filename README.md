...
- 外观设置支持实时调整圆角、玻璃透明度、边框强度、字体大小和模糊强度
...
| `npm test` | 运行任务状态、年度统计、AI URL、外观配置、提醒和加密兼容回归检查 |
...
- 任务量热力图按照任务所属日期统计：跨天任务覆盖范围内每天；只有开始时间时归属开始日；只有截止时间时沿用创建日至截止日范围；无计划时间时使用完成日或创建日。
- 完成量热力图按照任务的 `doneAt` 完成日期统计。
...
- 透明度、边框、字体和模糊效果分别使用 `--ui-glass-opacity`、`--ui-border-strength`、`--ui-font-scale`、`--ui-glass-blur` 和 `--ui-glass-blur-light`。
